...
void help_version ()
{
    cout << "COMPASS Pre-processor version 1.2" << endl << endl;
}
void help_usage ()
{
//Usage======================================================================
QString usage ("compass_pp -alt=ALT_NAME -hydro=HYDSIM_NAME -scenario=SCN_NAME \n"); 
usage.append  ("    [ -rules=RULE_FILE_NAME ] [ -hydsim=HYDSIM_PRT [-d] ] [ -spill=DAY_SPILL ]\n");
usage.append  ("    [ -dam=DAM_PARAMS ] [ -mod=ALL|NOSPILL ] [ -trans=MAF|FLOW|TEMP ]\n");
usage.append  ("    [ -debug=TEMP|SPILL|ALL ] [ -data=DATA_DIR ] [ -out=OUTPUT_DIR ] [ -help ]\n");
...
verbose.append ("                 part of the files in the 'step3' directory. if not provided,\n");
verbose.append ("                 step3 is not completed. No spaces allowed in name.\n");
...
int main (int argc, char *argv[])
{
    ...
    /* If starting with the HydSim output file,
     * 1) read the file,
     * 2) modulate flow if not already daily flow numbers, and
     * 3) place output csv file in correct dir. */
	if (!cmd_args->hydsim.isEmpty())
	{
        findFile(cmd_args->hydsim, dirs.hydsim, dirs.data);
        if (!cmd_args->dayspill.isEmpty())
        {
            findFile(cmd_args->dayspill, dirs.hydsim, dirs.data);
        }
    }
//        cout << "Processing HydSim file " << filename.toUtf8().data() << endl;
    if (!cmd_args->hydsim.isEmpty())
    {
        QProcess flowproc;
        QString cmd_line;
//        cmd_line = QString(dirs.bin + PATHSEP);
        ...
        cmd_line.append (cmd_args->hydsim);
		if (!cmd_args->dayspill.isEmpty())
		{
            cmd_line.append ("\" -p \"");
...
int get_args (int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
        ...
        else if (arg.compare ("-d", Qt::CaseInsensitive) == 0)
        {
            ret = 4;
            cmd_args->daily = true;
        }
    }
//    if (cmd_args->mod != NONE)
...
void setDirs()
{
    ...
    if (!cmd_args->dataDir.isEmpty())
        dirs.data = QString(cmd_args->dataDir);
    else
        dirs.data = QString(dirs.app + QString("/pre-proc"));

    if (!cmd_args->outputDir.isEmpty())
        dirs.user = QString(cmd_args->outputDir);
    else
        dirs.user = QString(dirs.data);

    sub = QString (return_option(cmd_args->mod));
    sub = cmd_args->mod == NONE? PATHSEP: QString(QString("%1%2%1").arg(PATHSEP, sub));
    temp = QString (QString("%1%2%3%2temp%4").arg
                    (dirs.user, PATHSEP, cmd_args->scenario, sub));
//#ifdef WIN32
//#endif
//	dirs.user.append (PATHSEP);
//	dirs.user.append (QString ("/temp/compass_pp"));
    dirs.bin       = QString(dirs.app);// + QString ("/bin");
    dirs.mean      = QString(QString("%1%2%3").arg(dirs.data, PATHSEP, QString("HistMean")));
    dirs.sub       = sub;
    dirs.final     = temp + QString("Final");//dirs.user + QString ("/Final") + dirs.sub + cmd_args->scenario;
    dirs.flfcast   = temp + QString("Work/FLOWFCAST");//dirs.user + QString ("/Work") + dirs.sub + cmd_args->scenario  + QString ("/FLOWFCAST");
    dirs.flarchive = temp + QString("Work/TEMPFCAST");//dirs.user + QString ("/Work") + dirs.sub + cmd_args->scenario  + QString ("/FLOWARCHIVE");
    dirs.tpfcast   = temp + QString("Work/TEMPFCAST");//dirs.user + QString ("/Work") + dirs.sub + cmd_args->scenario  + QString ("/TEMPFCAST");
    dirs.dat       = temp + QString("Work/DAT");//dirs.user + QString ("/Work") + dirs.sub + cmd_args->scenario + QString ("/DAT");
    dirs.hydsim    = temp + QString ("HydSim");//dirs.user + QString ("/HydSim");
	dirs.hydro     = dirs.hydsim + PATHSEP + cmd_args->hydro;
    dirs.monte     = QString(QString("%1%2datmaker").arg(dirs.data, PATHSEP));//dirs.data  + QString ("/datmaker");
    dirs.ppData    = QString(QString("%1%2PreProcData").arg(dirs.data, PATHSEP));//dirs.data  + QString ("/PreProcData");
    dirs.fmData    = QString(QString("%1%2FlowModData").arg(dirs.data, PATHSEP));//dirs.data  + QString ("/FlowModData");
    checkDir (dirs.data, false);
...
void runTempModel ()
{
    QProcess tempproc;
    int yearcount;
    QString flowfcastfile, oldYear, newYear, cmdline;
	cout << " ... Temperature processing ..." << endl;
    QDir::current().remove ("msg_preproc_temp.txt");
    QDir::current().remove ("msg_preproc_temp_err.txt");
    oldYear = QString::number(YEAR_INDEX);
    flowfcastfile = QString(QString("%1%2%3%4").arg(dirs.flfcast, PATHSEP, QString ("flowFC."), oldYear));
//    cmdline = QString (QString("%1%2%3").arg(dirs.bin, PATHSEP, QString ("temp_paulsen_r.exe")));
    cmdline = QString ("temp_paulsen_r.exe");
    cmdline.append(QString(" %1 %2 %3 %4 \n").arg(dirs.tpfcast, dirs.mean, oldYear, flowfcastfile));//arguments << dirs.tpfcast << dirs.mean << Year << flowfcastfile;
//        cmdline.append (QString(" 2>>msgs_temp_err.txt 1>>msgs_temp.txt"));
#ifdef Q_WS_WIN
        cmdline.replace ("/", "\\");
#endif
    tempproc.setStandardErrorFile(QString("msg_preproc_temp_err.txt"));
    tempproc.setStandardOutputFile(QString("msg_preproc_temp.txt"));

    for (yearcount = 0; yearcount < 70; yearcount++)
	{
        newYear =  QString::number(YEAR_INDEX + yearcount);
        flowfcastfile.replace(oldYear, newYear);
        cmdline.replace(oldYear, newYear);

/*        for (i = 0; i < cmdline.count(); i++)
            comline[i] = cmdline.at(i).toLatin1();
        comline[i] = '\0';*/
        cout << cmdline.toUtf8().data() << endl;
        tempproc.execute(cmdline);
//        system (comline);
/*        cout << cmdline.toUtf8().data() << endl;
        system (cmdline.toUtf8().data());*/
//        execl ("temp_paulsen_r.exe", dirs.tpfcast.toUtf8().data(), dirs.mean.toUtf8().data(), newYear.toUtf8().data(), flowfcastfile.toUtf8().data());
...
void getTemps ()
{
	...
	for (dam = ANA; dam < N_DAM; dam++)
	{
		...
		for (yr = 0; yr < 70; yr++)
		{
            Year = QString::number (YEAR_INDEX + yr);
            temp_file_name = QString(QString("%1%2%3%4.%5").arg(dirs.tpfcast, PATHSEP, "tempFC.", damAbbv, Year));
//			temp_file_name.append (QString ("/tempFC."));
//			temp_file_name.append (damAbbv);
//			temp_file_name.append (".");
//			temp_file_name.append (year));
...
{
    QFileInfo ruleInfo (cmd_args->rules);
    int okay = 0;
    if (ruleInfo.isReadable())
        okay = getRules (ruleInfo.path(), ruleInfo.fileName());
    else
        okay = getRules (dirs.ppData);
...
int writeCompassAlt (QString altName)
{
    ...
    // write the pointer file .compass-alts
    altFile.setFileName(".compass-alts");
    if (altFile.open (QIODevice::WriteOnly))
    {
        altFile.write (QString(QString(dirs.monte) + QString("\n")).toUtf8());
    }
...
void runCompass()
{
    ...
    outFilename = QString( QString("%1%2output.dat").arg(dirs.data, PATHSEP));
    compassAltsFilename = QString(QString("%1%2.compass-alts").arg(dirs.data, PATHSEP));
    compassAlts.setFileName(compassAltsFilename);
    compassAlts.copy (QString(".compass-alts"));
//    compassAltsFilename = dirs.data + PATHSEP + QString("datmaker/datmaker.alt");
//    compassAlts.setFileName(compassAltsFilename);
//    compassAlts.copy (QString("datmaker/datmaker.alt"));
//    compproc.execute(QString("copy ") + dirs.data + PATHSEP + QString(".compass-alts"));
//    compproc.execute(QString("copy ") + dirs.data + PATHSEP + QString("datmaker/datmaker.alt"));
//    compproc.execute(QString("copy ") + dirs.data + PATHSEP + QString("include.powerhouse"));

    if (cmd_args->debug != NONE)
	{
        etcFilename = QString(QString("%1%2debug_%3.etc").arg(
                                  dirs.data, PATHSEP, return_option(cmd_args->debug)));
/*		etcFilename.append ("debug_");
	...
	{
        etcFilename = QString(QString("%1%2base.etc").arg(
                                  dirs.data, PATHSEP));
	}
	//etcFile.setFileName (etcFilename);

    thisdir.cd (dirs.data);
	...
	for (year = 0; year < 70; year++)
	{
        ...
        arcFilename = dirs.data + PATHSEP + QString ("flow.archive");//arcFilename.section ('.', 0, -2);
        ...
        altFilename = dirs.dat + PATHSEP + QString ("summary.alt1.") + QString::number (year + YEAR_INDEX);
        sumFilename = dirs.dat + PATHSEP + QString ("summary.dat.") + QString::number (year + YEAR_INDEX);
...
//		cmdline = dirs.bin + PATHSEP;
        thisdir.remove (outFilename);
        thisdir.remove (QString("%1%2messages.txt").arg(dirs.data, PATHSEP));
        cmdline = QString(QString ("%1/compassb -bm -f %2/base.dat -c %3 -o %4\n").arg(
                              dirs.app, dirs.data, etcFilename, outFilename));
//        cmdline = QString (QString("compassb -lw -bmu -c %1/debug_all.etc -o output.dat \n").arg(
//                               cmd_args->dataDir));
...
#endif
        QFile messages;
        messages.setFileName (QString("%1%2messages.txt").arg(dirs.data, PATHSEP));//msgFilename);
		msgFile.open (QIODevice::WriteOnly);
        line = QString(QString("%1 ... Running COMPASS monte carlo mode with flow.archive\n").arg(
                           QString::number(year + YEAR_INDEX)));
/*        msgFile.write (QString::number (year + YEAR_INDEX).toUtf8());
        ...
        cout << cmdline.toUtf8().data();
        compproc.setStandardErrorFile(msgFile.fileName(), QIODevice::Append);
        compproc.execute(cmdline);
//        system (cmdline.toUtf8().data());
		thisdir.remove (datFilename);
        QFile::copy (outFilename, datFilename);

		// run COMPASS in scenario mode to write out summary.dat file
        if (cmd_args->debug != NONE)
		{
//			cmdline = QString ("cd ") + dirs.bin;
            cmdline = QString (QString ("%1/compassb -bsu -f %2 -c %3\n").arg(
                                   dirs.app, outFilename, etcFilename));
...
/*            msgFile.write (QString::number (year + YEAR_INDEX).toUtf8());
            ...
            msgFile.write (cmdline.toUtf8());*/
            line = QString(QString("%1 ... Running COMPASS scenario mode to create debug output\n").arg(
                               QString::number(year + YEAR_INDEX)));
            msgFile.write (line.toUtf8());
            msgFile.write (cmdline.toUtf8());
            cout << year + YEAR_INDEX << " ... Running COMPASS scenario mode to create debug output\n";
            cout << cmdline.toUtf8().data();
            compproc.execute(cmdline);

            copyFile ("summary.dat", &msgFile);
//            system (cmdline.toUtf8().data());
		}
...
//		cmdline = QString ("cd ") + dirs.bin;
        cmdline = QString(QString ("%1/compassb -bs -f %2/output.dat -c %2/base.etc -o %2/final.dat ").arg(dirs.bin, dirs.data));
        ...
        line = QString(QString("%1 ... Running COMPASS scenario mode to create final .dat file\n").arg(
                           QString::number(year + YEAR_INDEX)));
        msgFile.write (line.toUtf8());
        msgFile.write (cmdline.toUtf8());

        cout << line.toUtf8().data();
        cout << cmdline.toUtf8().data() << endl;
...
//		QFile::copy ("messages.txt", msgFilename);
        ...
        QFile::copy (QString("%1/final.dat").arg(dirs.data), finFilename);

/*        altFile.setFileName (dirs.data + QString("/summary.alt1"));
		if (altFile.exists())
		{
        ...
        sumFile.setFileName (dirs.data + QString("/summary.dat"));
		if (sumFile.exists())
		{
...
void create_final_files ()
{
    ...
    cmd = QString(QString ("compass_pp3 -name=%1 -input=\"%2\" -rule=%3 -dam=%4").arg(
                      cmd_args->scenario,
                      dirs.final,
                      cmd_args->altern,
                      cmd_args->damParam));
    cmdargs.append (QString ("-name=%1").arg(cmd_args->scenario));
    cmdargs.append(QString("-input=\"%1\"").arg(dirs.final));
    cmdargs.append(QString("-rule=%1").arg(cmd_args->altern));
    cmdargs.append(QString("-dam=%1").arg(cmd_args->damParam));
/*	cmdline.append (cmd_args->scenario);
	...
	if (!cmd_args->outputDir.isEmpty())
	{
        cmdargs.append(QString ("-output=\"%1\"").arg(cmd_args->outputDir));
/*		cmdline.append (" -output=\"");
		cmdline.append (cmd_args->outputDir);
        cmdline.append ("\" 2>msgs_final_err.txt ");*/
        cmd.append(QString(" -output=\"%1\"").arg(cmd_args->outputDir));
	}
...
#endif*/
    cmd.append("\n");
    cout << cmd.toUtf8().data();
/*    for (int i = 0; i < cmdargs.count(); i++)
        cout << " " << cmdargs.at(i).toUtf8().data();
    cout << endl;*/
    finalproc.setStandardOutputFile(QString("msg_preproc_final.txt"));
    finalproc.setStandardErrorFile(QString("msg_preproc_final_err.txt"));
    finalproc.execute(cmd);//, cmdargs);
//    system (cmdline.toUtf8().data());
}
...
void findFile (QString &filename, QString dir1, QString dir2)
{
    ...
    if (!fi.exists())
    {
        if (filename.contains(PATHSEP))
        {
            msg = QString(QString("File %1 does not exist.").arg(filename));
            help (msg.toUtf8().data());
        }

        fname = QString (dir1 + PATHSEP + filename);
        fi.setFile(fname);
        if (!fi.exists())
        {
            fname = QString (QString("%1%2%3%2%4").arg(
                       dir2, PATHSEP, QString("HydSim"), filename));
            fi.setFile(fname);
            if (!fi.exists())
            {
                msg = QString(QString("Cannot find file %1.").arg(filename));
                help (msg.toUtf8().data());
            }
        }
        filename = fname;
